  ...
  return useMutation({
    ...
    mutationFn: async (payload: DeveloperTeamAddNew) => {
      try {
        ...
        const response = await axios.post(
          `/dev-team-management/invitation/developer/create`,
          apiPayload
        );
    ...
    onSuccess: (data) => {
      ...
      if (data.data?.qrCode || data.data?.data?.qrCode) {
        console.log(
          "QR Code received:",
          data.data?.qrCode || data.data?.data?.qrCode
        );
      }
    },
    onError: (error: any) => {
...
export const useEditDeveloperTeamInvitationMutation = () => {
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      invitationId: string;
      payload: DeveloperTeamEditPayload;
    }) => {
      try {
  ...
  return useMutation({
    ...
    onSuccess: (data, { userId }) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["developerTeamProfile", userId],
      });
    },
    onError: (error: any) => {
...
export const useResendDeveloperTeamInvitationMutation = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (invitationId: string) => {
      try {
        console.log(
          "Resending developer team invitation for ID:",
          invitationId
        );

        // ใช้ API endpoint สำหรับ resend invitation
        const response = await axios.post(
          `/dev-team-management/invitation/developer/resend/${invitationId}`
        );
        ...
        if (error.response) {
          const errorMessage =
            error.response.data?.message ||
            `Resend failed: ${error.response.status}`;
          throw new Error(errorMessage);
        }
...
export const useDeleteDeveloperTeamMutation = () => {
  ...
  return useMutation({
    ...
    get isPending() {
      return (
        deleteInvitationMutation.isPending || deleteMemberMutation.isPending
      );
    },
    get variables() {
      return (
        deleteInvitationMutation.variables || deleteMemberMutation.variables
      );
    },
  });
};
